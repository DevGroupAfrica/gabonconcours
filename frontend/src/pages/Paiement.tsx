...
const Paiement = () => {
    ...
    const handlePayment = async () => {
        ...
        try {
            ...
            if (error.message === 'SERVICE_PAIEMENT_NON_CONFIGURE') errorMessage = "Le paiement mobile n’est pas encore configuré. Veuillez contacter l’administrateur.";
            else if (error.message === 'PORTEFEUILLE_SINGPAY_EN_ATTENTE') errorMessage = "Le portefeuille SingPay est en attente d’activation. Veuillez contacter l’administrateur.";
            else if (error.message === 'MONTANT_MYPVIT_TROP_BAS') errorMessage = "MyPVit exige un montant supérieur à 500 FCFA. Pour un succès en test, utilisez un montant entre 501 et 999 FCFA.";
            else if (error.message === 'NUMERO_OPERATEUR_INVALIDE') errorMessage = "Le numéro ne correspond pas à l’opérateur sélectionné.";
            else if (error.message.includes('NUPCAN')) errorMessage = "Erreur d'identification du candidat";
            ...
            else if (error.message.includes('refusé') || error.message.includes('expiré')) errorMessage = error.message;
