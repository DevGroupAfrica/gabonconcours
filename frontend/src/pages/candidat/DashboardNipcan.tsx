...
const DashboardNipcan = () => {
    ...
    return (
        <div className="candidate-shell min-h-screen bg-[#f5f8fc] text-slate-800">
            <aside className={`fixed inset-y-0 left-0 z-40 flex w-64 flex-col border-r border-slate-300 bg-white lg:translate-x-0 ${mobileMenu ? 'translate-x-0' : '-translate-x-full'}`}>
                <div className="flex h-16 items-center justify-between border-b border-slate-100 px-5">
                    <div>
                        <p className="text-lg font-bold tracking-tight text-slate-950">GABConcours<span className="text-primary">.</span></p>
                        <p className="mt-0.5 text-xs font-medium text-slate-500">Espace candidat</p>
                    </div>
                ...
                <div className="border-b border-slate-100 p-3">
                    ...
                    {data.candidatures.length > 0 ? (
                        <select
                            ...
                            className="w-full rounded-md border border-slate-300 bg-[#f8faff] px-3 py-2.5 text-sm font-medium outline-none focus:border-primary"
                        >
                    ...
                    <Button className="mt-3 w-full" variant="outline" size="sm" onClick={() => navigate('/concours')}>
                    ...
                    {menu.map(item => {
                        ...
                        return (
                            <button
                                ...
                                className={`flex w-full items-center gap-3 rounded-md border-l-2 px-3 py-2 text-left text-sm font-medium disabled:cursor-not-allowed disabled:opacity-40 ${
                                    activeTab === item.id ? 'border-primary bg-[#edf4ff] text-[#2459b8]' : 'border-transparent text-slate-600 hover:bg-[#f4f7fb] hover:text-slate-900'
                                }`}
                            >
                ...
                <div className="border-t border-slate-100 p-3">
                    ...
                    <button onClick={() => {localStorage.removeItem('candidat_nipcan'); navigate('/connexion');}} className="flex w-full items-center gap-3 rounded-md px-3 py-2.5 text-sm font-medium text-slate-600 hover:bg-slate-100 hover:text-red-700">
            ...
            <div className="lg:pl-64">
                <header className="sticky top-0 z-30 flex h-16 items-center justify-between border-b border-slate-300 bg-white px-4 lg:px-6">
                    <div className="flex items-center gap-3">
                        <button className="lg:hidden" onClick={() => setMobileMenu(true)}><Menu className="h-5 w-5"/></button>
                        <div>
                            <h1 className="font-semibold">{pageTitle(activeTab)}</h1>
                            <p className="hidden text-xs text-slate-400 sm:block">{selected ? `${selected.concours.libcnc} · ${selected.nupcan}` : 'Espace personnel'}</p>
                        </div>
                    </div>
                    <Button variant="outline" size="sm" onClick={loadDashboard}><RefreshCw className="h-4 w-4"/>Actualiser</Button>
                </header>
...
const Overview = ({data, selected, setTab, navigate}: any) => (
    <div className="space-y-4">
        <div className="rounded-md border border-[#dbe7f6] bg-[#fbfdff] px-5 py-5 sm:px-6 sm:py-6">
            <p className="text-sm font-semibold text-primary">Bonjour {data.candidat.prncan}</p>
            <h2 className="mt-2 max-w-3xl text-2xl font-semibold leading-tight tracking-tight text-slate-950">Suivez votre dossier et réalisez les prochaines étapes.</h2>
            <p className="mt-2 max-w-2xl text-sm leading-5 text-slate-500">Toutes les informations utiles de votre candidature sont regroupées ici.</p>
            <div className="mt-4 flex flex-wrap gap-2">
                <Button onClick={() => selected && setTab('documents')} disabled={!selected}><FileText className="h-4 w-4"/>Gérer les documents</Button>
                <Button variant="outline" onClick={() => navigate('/concours')}><Plus className="h-4 w-4"/>Nouvelle candidature</Button>
            </div>
        </div>
...
const ApplicationCard = ({application, setTab, navigate}: any) => (
    <div className="overflow-hidden rounded-xl border border-[#dbe7f6] bg-white shadow-[0_12px_35px_rgba(35,67,110,.06)]">
        <div className="flex flex-col gap-3 border-b border-slate-100 bg-gradient-to-r from-[#fbfdff] to-white p-5 sm:flex-row sm:items-start sm:justify-between">
            <div><span className={`inline-flex rounded-full px-2.5 py-1 text-[11px] font-bold uppercase tracking-wide ${application.progression === 100 ? 'bg-emerald-50 text-emerald-700' : 'bg-blue-50 text-primary'}`}>{application.progression === 100 ? 'Dossier transmis' : 'En préparation'}</span><h3 className="mt-3 text-xl font-semibold">{application.concours.libcnc}</h3><p className="mt-1 text-sm text-slate-500">{application.filiere.nomfil} · {application.concours.etablissement}</p></div>
        ...
        <div className="px-5 pb-5">
            ...
            <div className="h-2 overflow-hidden rounded-full bg-slate-100"><div className={`h-full rounded-full ${application.progression === 100 ? 'bg-emerald-500' : 'bg-primary'}`} style={{width: `${application.progression}%`}}/></div>
...
const ProgressStep = ({done, label}: any) => <div className={`flex items-center gap-2 rounded-md border px-3 py-2 text-xs font-semibold ${done ? 'border-emerald-100 bg-emerald-50 text-emerald-700' : 'border-slate-200 bg-slate-50 text-slate-400'}`}><CheckCircle2 className="h-3.5 w-3.5"/>{label}</div>;
...
const Profile = ({data, setTab, navigate}: any) => {
    ...
    return (
        <div className="mx-auto w-full max-w-6xl space-y-5">
            <section className="relative overflow-hidden rounded-2xl bg-[#172033] text-white shadow-[0_18px_45px_rgba(31,48,74,.13)]">
                ...
                <div className="relative grid gap-8 p-6 sm:p-8 lg:grid-cols-[1fr_auto] lg:items-end lg:p-10">
                    <div className="flex flex-col gap-5 sm:flex-row sm:items-center">
                        ...
                        <div>
                            <span className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/5 px-3 py-1 text-[11px] font-semibold text-emerald-300">
                                <ShieldCheck className="h-3.5 w-3.5"/>Profil candidat actif
                            </span>
                            <h2 className="mt-4 text-3xl font-bold tracking-tight">{data.candidat.prncan} {data.candidat.nomcan}</h2>
                            <p className="mt-2 text-sm text-slate-300">Votre identité et vos candidatures sont regroupées dans cet espace personnel.</p>
                            <div className="mt-4 inline-flex items-center gap-2 rounded-md bg-white/5 px-3 py-2 font-mono text-xs text-slate-300">
                                <Fingerprint className="h-4 w-4 text-[#63d29a]"/>{data.candidat.nipcan}
                            </div>
                        </div>
                    </div>
            ...
            <div className="grid gap-5 lg:grid-cols-[1.05fr_0.95fr]">
                <section className="overflow-hidden rounded-xl border border-[#dbe7f6] bg-white shadow-[0_12px_35px_rgba(35,67,110,.05)]">
                    <div className="flex items-start justify-between gap-4 border-b border-slate-100 px-6 py-5">
                        <div>
                            <p className="text-xs font-bold uppercase tracking-[0.16em] text-[#159a62]">Informations personnelles</p>
                        ...
                        <div className="hidden h-10 w-10 items-center justify-center rounded-lg bg-emerald-50 text-[#159a62] sm:flex">
                    ...
                    {recentApplication ? (
                        <div className="p-6">
                            <div className="flex items-start gap-4">
                                <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-[#edf4ff] text-primary">
                            ...
                            <div className="mt-2 h-2 overflow-hidden rounded-full bg-slate-100">
                                <div className={`h-full rounded-full ${recentApplication.progression === 100 ? 'bg-emerald-500' : 'bg-primary'}`} style={{width: `${recentApplication.progression}%`}}/>
                            </div>
...
const ProfileStat = ({value, label}: any) => <div className="min-w-[88px] bg-white/5 px-4 py-4 text-center"><p className="text-xl font-bold">{value}</p><p className="mt-1 text-[10px] font-semibold uppercase tracking-[0.12em] text-slate-400">{label}</p></div>;
const ProfileInfo = ({icon: Icon, label, value, verified = false}: any) => <div className="rounded-lg border border-slate-200 bg-[#fbfdff] p-4"><div className="flex items-start justify-between gap-3"><div className="flex h-9 w-9 items-center justify-center rounded-lg bg-white text-primary shadow-sm"><Icon className="h-4 w-4"/></div>{verified && <span className="inline-flex items-center gap-1 text-[10px] font-semibold uppercase tracking-wide text-emerald-600"><CheckCircle2 className="h-3 w-3"/>Vérifié</span>}</div><p className="mt-4 text-xs text-slate-500">{label}</p><p className="mt-1 break-all text-sm font-semibold text-slate-800">{value || 'Non renseigné'}</p></div>;
...
const Metric = ({label, value, icon: Icon}: any) => <div className="rounded-md border border-[#dbe7f6] bg-white p-4"><div className="flex items-center justify-between"><div><p className="text-xs font-medium text-slate-500">{label}</p><p className="mt-1.5 text-2xl font-semibold text-[#29415f]">{value}</p></div><div className="flex h-8 w-8 items-center justify-center rounded-md bg-[#edf4ff]"><Icon className="h-4 w-4 text-primary"/></div></div></div>;
